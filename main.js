...
function showToast(msg) {
    ...
    if(toast){ toast.textContent = msg; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 1500); }
}
...
function copyTextToClipboard(text, msg) {
    ...
    catch (e) { if (navigator.clipboard) { navigator.clipboard.writeText(text).then(() => showToast(msg || '복사되었습니다.')); } }
...
window.toggleSection = function(sectionId, btnElement) {
    ...
    if (isHidden) { section.classList.remove('hidden'); section.style.display = 'block'; if (btnElement) { let txt = btnElement.innerHTML; if(txt.includes('보기')) btnElement.innerHTML = txt.replace('보기', '접기'); else if(txt.includes('펴기')) btnElement.innerHTML = txt.replace('펴기', '접기').replace('▼', '▲'); else btnElement.innerHTML = '접기 ▲'; } if (sectionId === 'sms-parser-section') { const smsInput = document.getElementById('sms-input'); if (smsInput) { smsInput.style.height = 'auto'; smsInput.style.height = (smsInput.scrollHeight) + 'px'; } } } 
    else { section.classList.add('hidden'); section.style.display = 'none'; if (btnElement) { let txt = btnElement.innerHTML; if(txt.includes('접기')) btnElement.innerHTML = txt.replace('접기', '보기').replace('▲', '▼'); else btnElement.innerHTML = '펴기 ▼'; } }
};

function toggleUI() {
    const typeSelect = document.getElementById('type'); const editModeIndicator = document.getElementById('edit-mode-indicator'); const smsSection = document.getElementById('sms-parser-section');
...
// ==========================================
// PART 3: MANAGERS (CENTER, SALARY, FIXED)
// ==========================================
...
window.editCenter = function(oldName) { const loc = MEM_LOCATIONS[oldName] || {}; const newName = prompt("지역 이름 수정:", oldName); if (newName === null || newName.trim() === "") return; const newAddr = prompt("주소 수정:", loc.address || ""); if (newAddr === null) return; const newMemo = prompt("메모 수정:", loc.memo || ""); if (newMemo === null) return; if (newName !== oldName) { const idx = MEM_CENTERS.indexOf(oldName); if (idx > -1) MEM_CENTERS[idx] = newName; MEM_CENTERS.sort(); delete MEM_LOCATIONS[oldName]; } MEM_LOCATIONS[newName] = { address: newAddr, memo: newMemo }; saveData(); displayCenterList(document.getElementById('center-search-input')?.value || ''); showToast("수정되었습니다."); };
...
window.addNewCenterFromInput = function() { let input = document.getElementById('add-center-input') || document.getElementById('new-center-name'); if (!input) { const settingsPage = document.getElementById('settings-page'); if (settingsPage) { const inputs = settingsPage.querySelectorAll('input[type="text"]'); for(let i=0; i<inputs.length; i++) { if(inputs[i].placeholder && (inputs[i].placeholder.includes('지역') || inputs[i].placeholder.includes('이름'))) { input = inputs[i]; break; } } } } if (!input) return alert("입력창을 찾을 수 없습니다."); const name = input.value.trim(); if (!name) return alert("지역 이름을 입력해주세요."); if (MEM_CENTERS.includes(name)) { alert("이미 등록된 지역입니다."); input.value = ""; return; } updateLocationData(name, "", ""); input.value = ""; displayCenterList(document.getElementById('center-search-input')?.value || ''); showToast(`'${name}' 지역이 추가되었습니다.`); };
...
function displayFixedExpenseManager() {
    let container = document.getElementById('expense-management-container'); if (!container) { const parent = document.getElementById('data-management-body'); if (!parent) return; container = document.createElement('div'); container.id = 'expense-management-container'; container.style.marginTop = '20px'; container.style.borderTop = '1px solid #eee'; container.style.paddingTop = '15px'; const salaryContainer = document.getElementById('salary-management-container'); if (salaryContainer) salaryContainer.parentNode.insertBefore(container, salaryContainer.nextSibling); else parent.appendChild(container); }
...
window.saveFixedExpense = function() { const month = parseInt(document.getElementById('fe-month').value) || 0; const day = parseInt(document.getElementById('fe-day').value); const name = document.getElementById('fe-name').value.trim(); const memo = document.getElementById('fe-memo').value.trim(); const cost = parseInt(document.getElementById('fe-cost').value.replace(/[^0-9]/g, '')); const editIdx = parseInt(document.getElementById('fe-edit-index').value); if (!name) { alert("항목 이름을 입력해주세요."); return; } if (isNaN(cost) || cost <= 0) { alert("금액을 입력해주세요."); return; } const newCheck = { month, day, name, memo, cost }; if (editIdx > -1) { MEM_FIXED_EXPENSES[editIdx] = newCheck; showToast("수정되었습니다."); } else { MEM_FIXED_EXPENSES.push(newCheck); showToast("추가되었습니다."); } if(!MEM_EXPENSE_ITEMS.includes(name)) { MEM_EXPENSE_ITEMS.push(name); MEM_EXPENSE_ITEMS.sort(); } saveData(); displayFixedExpenseManager(); if(typeof updateAllDisplays === 'function') updateAllDisplays(); };
window.editFixedExpense = function(index) { const item = MEM_FIXED_EXPENSES[index]; if (!item) return; document.getElementById('fe-month').value = item.month || 0; document.getElementById('fe-day').value = item.day; document.getElementById('fe-name').value = item.name; document.getElementById('fe-memo').value = item.memo; document.getElementById('fe-cost').value = item.cost; document.getElementById('fe-edit-index').value = index; const btn = document.getElementById('fe-save-btn'); btn.textContent = "수정"; btn.style.background = "#17a2b8"; showToast("수정 모드: 내용을 변경하고 저장하세요."); };
window.deleteFixedExpense = function(index) { if(!confirm("이 고정 지출 항목을 삭제하시겠습니까?")) return; MEM_FIXED_EXPENSES.splice(index, 1); saveData(); displayFixedExpenseManager(); if(typeof updateAllDisplays === 'function') updateAllDisplays(); showToast("삭제되었습니다."); };
// ==========================================
// PART 4: SMS & OCR FUNCTIONS
// ==========================================
...
async function performOCR(file) {
    ...
    if (!statusDiv || !resultContainer) return; statusDiv.textContent = "영수증 분석 중... (5~10초 소요)"; resultContainer.classList.add('hidden');
...
function createSummaryHTML(title, records) {
    ...
    let totalFuelCost = 0, totalFuelLiters = 0, totalEmptyDist = 0;
    records.forEach(r => { 
    ...
    const items = [ 
        { label: '수입', value: formatToManwon(totalIncome) + ' 만원', className: 'income' }, 
        { label: '지출', value: formatToManwon(totalExpense) + ' 만원', className: 'cost' }, 
        { label: '정산', value: formatToManwon(netIncome) + ' 만원', className: 'net' }, 
        { label: '운행거리', value: totalDistance.toFixed(1) + ' km', className: '' }, 
        { label: '운행건수', value: totalTripCount + ' 건', className: '' }, 
        { label: '주유금액', value: formatToManwon(totalFuelCost) + ' 만원', className: 'cost' }, 
        { label: '주유리터', value: totalFuelLiters.toFixed(2) + ' L', className: '' }, 
...
function displayTodayRecords(date) {
    ...
    const oldTbody = todayTable.querySelector('tbody'); if(oldTbody) oldTbody.remove();
    ...
    displayList.forEach(r => {
        if (['화물운송','대기','운행취소','공차이동'].includes(r.type)) {
            ...
            if (idx > -1 && idx < sortedAll.length - 1) { 
                ...
                if(next.type !== '운행취소') isCompleted = true; 
            }
    ...
    const createRow = (r, isHidden = false) => {
        ...
        let timeDisplay = r.time || '<span style="color:#aaa;">(대기)</span>'; 
        if(r.time && r.date !== date) timeDisplay = `<span style="font-size:0.8em; color:#888;">(익일)</span> ${r.time}`;
        ...
        if (r.time) { 
            ...
            if (idx > -1 && idx < sortedAll.length - 1) { 
                const next = sortedAll[idx + 1]; 
                if(next.type !== '운행취소') { 
                    endTime = (next.date !== r.date) ? `(${next.date.substring(5)}) ${next.time}` : next.time; 
                    const diff = new Date(`${next.date}T${next.time}`) - new Date(`${r.date}T${r.time}`); 
                    if (diff >= 0) duration = `${Math.floor(diff / 3600000)}h ${Math.floor((diff % 3600000) / 60000)}m`; 
                } 
            } 
        ...
        let money = ''; 
        const inc = safeInt(r.income); const cst = safeInt(r.cost); 
    ...
    if(todaySummaryDiv) todaySummaryDiv.innerHTML = createSummaryHTML('오늘의 기록', dayRecords);
}
...
function displayWeeklyRecords() {
    ...
    const year = yearSelect.value, month = monthSelect.value, selectedPeriod = `${year}-${month}`;
    const weeklyTbody = document.querySelector('#weekly-summary-table tbody'); if(weeklyTbody) weeklyTbody.innerHTML = '';
    const monthRecords = MEM_RECORDS.filter(r => getStatisticalDate(r.date, r.time).startsWith(selectedPeriod));
