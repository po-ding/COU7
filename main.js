// ==========================================
// PART 1: GLOBAL FUNCTIONS & DATA MANAGEMENT
// ==========================================

// 0. GLOBAL UI CONTROLLERS
function controlSectionByText(keyword, show) {
    const knownIds = { 
        '기록 일시': ['basic-info-section', 'datetime-info-fieldset', 'date-fieldset'], 
        '기록 종류': ['type-fieldset'], 
        '금액 정보': ['cost-info-fieldset'] 
    };
    if(knownIds[keyword]) { 
        knownIds[keyword].forEach(id => { 
            const el = document.getElementById(id); 
            if(el) { 
                el.style.display = show ? 'block' : 'none'; 
                if(!show) el.classList.add('hidden'); else el.classList.remove('hidden'); 
            } 
        }); 
    }
    const labels = document.querySelectorAll('legend, h3, h4, h5, label, span.section-title');
    labels.forEach(el => { 
        if (el.textContent.trim().includes(keyword)) { 
            const container = el.closest('fieldset') || el.closest('.section') || el.parentElement; 
...
window.toggleSection = function(sectionId, btnElement) {
    ...
    if (isHidden) {
        ...
        if (btnElement) { 
            let txt = btnElement.innerHTML; 
            if(txt.includes('보기')) btnElement.innerHTML = txt.replace('보기', '접기'); 
            else if(txt.includes('펴기')) btnElement.innerHTML = txt.replace('펴기', '접기').replace('▼', '▲'); 
            else btnElement.innerHTML = '접기 ▲'; 
        }
        if (sectionId === 'sms-parser-section') { 
            const smsInput = document.getElementById('sms-input'); 
            if (smsInput) { smsInput.style.height = 'auto'; smsInput.style.height = (smsInput.scrollHeight) + 'px'; } 
        }
    } else {
        section.classList.add('hidden'); section.style.display = 'none';
        if (btnElement) { 
            let txt = btnElement.innerHTML; 
            if(txt.includes('접기')) btnElement.innerHTML = txt.replace('접기', '보기').replace('▲', '▼'); 
            else btnElement.innerHTML = '펴기 ▼'; 
        }
    }
};
...
window.openSettings = function() {
    ...
    const btnGo = document.getElementById('go-to-settings-btn'); 
    ...
    try {
        ...
        if (typeof displayCenterList === 'function') displayCenterList(searchInput ? searchInput.value : '');
        ...
        const limitInput = document.getElementById('subsidy-limit'); 
        const limitBtn = document.getElementById('subsidy-save-btn');
        if(limitInput) limitInput.parentElement.style.display = 'none'; 
...
window.editCenter = function(oldName) {
    ...
    const newName = prompt("지역 이름 수정:", oldName); 
    ...
    if (newName !== oldName) { 
        const idx = MEM_CENTERS.indexOf(oldName); 
        if (idx > -1) MEM_CENTERS[idx] = newName; 
        MEM_CENTERS.sort(); 
        delete MEM_LOCATIONS[oldName]; 
    }
    MEM_LOCATIONS[newName] = { address: newAddr, memo: newMemo };
    saveData(); 
...
window.addNewCenterFromInput = function() {
    ...
    if (!input) { 
        const settingsPage = document.getElementById('settings-page'); 
        if (settingsPage) { 
            const inputs = settingsPage.querySelectorAll('input[type="text"]'); 
            for(let i=0; i<inputs.length; i++) { 
                if(inputs[i].placeholder && (inputs[i].placeholder.includes('지역') || inputs[i].placeholder.includes('이름'))) { 
    ...
    if (!name) return alert("지역 이름을 입력해주세요.");
    if (MEM_CENTERS.includes(name)) { alert("이미 등록된 지역입니다."); input.value = ""; return; }
    
    updateLocationData(name, "", ""); 
    input.value = ""; 
    displayCenterList(document.getElementById('center-search-input')?.value || ''); 
    showToast(`'${name}' 지역이 추가되었습니다.`);
};

// ==========================================
// SALARY & FIXED EXPENSE MANAGER
// ==========================================
...
function displaySalaryManager() {
    ...
    if (!container) {
        const parent = document.getElementById('data-management-body'); if (!parent) return;
        container = document.createElement('div'); container.id = 'salary-management-container'; 
        container.style.marginTop = '20px'; container.style.borderTop = '1px solid #eee'; container.style.paddingTop = '15px';
    ...
    let listHtml = '';
    Object.keys(MEM_SALARIES).sort().reverse().forEach(key => {
        const amount = MEM_SALARIES[key];
...
window.saveSalary = function() {
    const y = document.getElementById('salary-set-year').value; 
    const m = document.getElementById('salary-set-month').value; 
    const amountInput = document.getElementById('salary-set-amount'); 
    ...
    if (typeof MEM_SALARIES === 'undefined') MEM_SALARIES = {};
    const key = `${y}-${m}`; MEM_SALARIES[key] = amount;
    
    saveData(); 
    displaySalaryManager(); 
    if (typeof updateAllDisplays === 'function') updateAllDisplays();
    showToast(`${y}년 ${m}월 급여가 저장되었습니다.`); 
    amountInput.value = ''; amountInput.blur();
};

window.editSalary = function(key, amount) {
    const [year, month] = key.split('-'); 
    const yEl = document.getElementById('salary-set-year'); 
    const mEl = document.getElementById('salary-set-month'); 
    const aEl = document.getElementById('salary-set-amount');
    if(yEl && mEl && aEl) { 
        yEl.value = year; mEl.value = month; aEl.value = amount; 
        aEl.scrollIntoView({ behavior: 'smooth', block: 'center' }); 
        setTimeout(() => { aEl.focus(); }, 300); 
        showToast(`${year}년 ${month}월 수정 모드`); 
    }
};

window.deleteSalary = function(key) {
    if(!confirm(`${key} 급여 내역을 삭제하시겠습니까?`)) return;
    if (typeof MEM_SALARIES !== 'undefined') { 
        delete MEM_SALARIES[key]; 
        saveData(); 
        displaySalaryManager(); 
        if (typeof updateAllDisplays === 'function') updateAllDisplays(); 
        showToast('삭제되었습니다.'); 
...
// [고정 지출 관리 (Fixed Expense Manager)] - 월 선택 추가
function displayFixedExpenseManager() {
    let container = document.getElementById('expense-management-container');
    if (!container) {
        const parent = document.getElementById('data-management-body'); if (!parent) return;
        container = document.createElement('div'); container.id = 'expense-management-container'; 
        container.style.marginTop = '20px'; container.style.borderTop = '1px solid #eee'; container.style.paddingTop = '15px';
        const salaryContainer = document.getElementById('salary-management-container'); 
        if (salaryContainer) salaryContainer.parentNode.insertBefore(container, salaryContainer.nextSibling); 
        else parent.appendChild(container);
    }
    
    // 월 옵션 (0: 매월, 1~12: 특정 월)
    let monthOpts = `<option value="0">매월 (반복)</option>`;
    for(let i=1; i<=12; i++) monthOpts += `<option value="${i}">${i}월</option>`;
    ...
    if (MEM_FIXED_EXPENSES.length === 0) {
        listHtml += `<div style="padding:15px; text-align:center; color:#999;">등록된 고정 지출이 없습니다.<br>(예: 매월 25일 보험료 10만원)</div>`;
    } else {
        // 정렬: 매월(0) -> 월별(1~12) -> 일자순
        MEM_FIXED_EXPENSES.sort((a,b) => {
            const mA = a.month || 0; const mB = b.month || 0;
            if(mA !== mB) return mA - mB;
            return a.day - b.day;
        });

        MEM_FIXED_EXPENSES.forEach((item, index) => {
            const periodStr = (item.month && item.month > 0) ? `<span style="color:#d63384;">[매년 ${item.month}월]</span>` : `<span style="color:#007bff;">[매월]</span>`;
            listHtml += `
                <div style="background:white; padding:10px; border-bottom:1px solid #eee; display:flex; justify-content:space-between; align-items:center;">
                    <div style="flex:1;">
                        <div style="font-weight:bold; color:#333;">
                            ${periodStr} ${item.day}일 - ${item.name}
                        </div>
                        <div style="font-size:0.85em; color:#666; margin-top:2px;">
                            ${item.memo ? `📝 ${item.memo} | ` : ''} 
                            <strong style="color:#333;">${formatToManwon(item.cost)} 만원</strong>
                        </div>
                    </div>
                    <div style="display:flex; gap:5px; margin-left:5px;">
                        <button type="button" onclick="window.editFixedExpense(${index})" style="background:#17a2b8; color:white; border:none; border-radius:4px; padding:6px 10px; font-size:0.8em;">수정</button>
                        <button type="button" onclick="window.deleteFixedExpense(${index})" style="background:#dc3545; color:white; border:none; border-radius:4px; padding:6px 10px; font-size:0.8em;">삭제</button>
                    </div>
                </div>`;
        });
    }
    ...
    container.innerHTML = `
        <h4 style="margin-bottom:10px; font-weight:bold; color:#333; font-size:1.1em;">💸 고정 지출 관리 (매월/연간)</h4>
        
        <div style="background:#fff0f0; padding:10px; border-radius:5px; border:1px solid #f5c6cb; margin-bottom:10px;">
            <div style="display:flex; gap:5px; margin-bottom:5px;">
                <select id="fe-month" style="width:110px; padding:8px; border:1px solid #ccc; border-radius:4px;">${monthOpts}</select>
                <select id="fe-day" style="width:70px; padding:8px; border:1px solid #ccc; border-radius:4px;">${dayOpts}</select>
                <input type="text" id="fe-name" placeholder="항목명 (예: 보험료)" style="flex:1; padding:8px; border:1px solid #ccc; border-radius:4px;">
            </div>
            <div style="display:flex; gap:5px; margin-bottom:5px;">
                <input type="text" id="fe-memo" placeholder="메모 (선택)" style="flex:1; padding:8px; border:1px solid #ccc; border-radius:4px;">
            </div>
            <div style="display:flex; gap:5px;">
                <input type="tel" id="fe-cost" inputmode="numeric" placeholder="금액 (원)" oninput="this.value=this.value.replace(/[^0-9]/g,'')" style="flex:1; padding:8px; border:1px solid #007bff; border-radius:4px; font-weight:bold;">
                <button type="button" id="fe-save-btn" onclick="window.saveFixedExpense()" style="width:60px; background:#28a745; color:white; border:none; border-radius:4px; font-weight:bold;">추가</button>
            </div>
            <input type="hidden" id="fe-edit-index" value="-1">
        </div>
...
window.saveFixedExpense = function() {
    const month = parseInt(document.getElementById('fe-month').value) || 0;
    const day = parseInt(document.getElementById('fe-day').value);
    const name = document.getElementById('fe-name').value.trim();
    const memo = document.getElementById('fe-memo').value.trim();
    const cost = parseInt(document.getElementById('fe-cost').value.replace(/[^0-9]/g, ''));
    const editIdx = parseInt(document.getElementById('fe-edit-index').value);

    if (!name) { alert("항목 이름을 입력해주세요."); return; }
    if (isNaN(cost) || cost <= 0) { alert("금액을 입력해주세요."); return; }

    const newCheck = { month, day, name, memo, cost }; // month 0: 매월

    if (editIdx > -1) {
        MEM_FIXED_EXPENSES[editIdx] = newCheck;
        showToast("수정되었습니다.");
    } else {
        MEM_FIXED_EXPENSES.push(newCheck);
        showToast("추가되었습니다.");
    }
    ...
    saveData();
    displayFixedExpenseManager();
    if(typeof updateAllDisplays === 'function') updateAllDisplays();
};

window.editFixedExpense = function(index) {
    const item = MEM_FIXED_EXPENSES[index];
    if (!item) return;
    document.getElementById('fe-month').value = item.month || 0;
    document.getElementById('fe-day').value = item.day;
    document.getElementById('fe-name').value = item.name;
    document.getElementById('fe-memo').value = item.memo;
    document.getElementById('fe-cost').value = item.cost;
    document.getElementById('fe-edit-index').value = index;
    const btn = document.getElementById('fe-save-btn');
    btn.textContent = "수정";
    btn.style.background = "#17a2b8";
    showToast("수정 모드: 내용을 변경하고 저장하세요.");
};

window.deleteFixedExpense = function(index) {
    if(!confirm("이 고정 지출 항목을 삭제하시겠습니까?")) return;
    MEM_FIXED_EXPENSES.splice(index, 1);
    saveData();
    displayFixedExpenseManager();
    if(typeof updateAllDisplays === 'function') updateAllDisplays();
    showToast("삭제되었습니다.");
...
function copyTextToClipboard(text, msg) {
    ...
    try { 
        ...
        if (navigator.clipboard) { 
            navigator.clipboard.writeText(text).then(() => showToast(msg || '주소가 복사되었습니다.')).catch(err => console.log('복사 실패:', err)); 
        } 
    }
...
function toggleUI() {
    ...
    const isEditMode = editModeIndicator && !editModeIndicator.classList.contains('hidden');
    const sections = ['fuel-details', 'supply-details', 'expense-details', 'trip-actions', 'general-actions', 'edit-actions'];
    ...
    const costInfo = document.getElementById('cost-info-fieldset');
    const costWrapper = document.getElementById('cost-wrapper');
    const incomeWrapper = document.getElementById('income-wrapper');

    if (type === '운행종료') {
        if(transportDetails) transportDetails.classList.add('hidden');
        if(costInfo) costInfo.classList.add('hidden');
        controlSectionByText('기록 일시', false);
        controlSectionByText('기록 종류', false);
        return;
    }

    if (isEditMode) {
        if(smsSection) smsSection.classList.add('hidden'); 
        if(document.getElementById('edit-actions')) document.getElementById('edit-actions').classList.remove('hidden');
    } else {
    ...
    if (type === '화물운송' || type === '대기') {
        ...
        if (type === '주유소') document.getElementById('fuel-details')?.classList.remove('hidden');
            ...
            const expSection = document.getElementById('expense-details');
            if(expSection) {
                expSection.classList.remove('hidden');
                if(!document.getElementById('expense-memo')) {
                    const div = document.createElement('div');
                    div.style.marginTop = '10px';
                    div.innerHTML = `<label style="display:block; color:#666; font-size:0.9em; margin-bottom:4px;">메모 (선택)</label><input type="text" id="expense-memo" placeholder="비고/상세내용" style="width:100%; padding:10px; border:1px solid #ddd; border-radius:4px;">`;
...
function displayCenterList(filter='') {
    ...
    list.forEach(c => {
        ...
        const locInfo = MEM_LOCATIONS[c] || {};
        const infoText = locInfo.address ? `${c} (${locInfo.address})` : c;
        div.innerHTML=`<div class="info"><span class="center-name">${infoText}</span><div class="action-buttons"><button type="button" class="center-action-btn btn-edit" style="background:#17a2b8;" onclick="window.editCenter('${c}')">수정</button><button type="button" class="center-action-btn btn-delete" style="background:#dc3545;" onclick="window.deleteCenter('${c}')">삭제</button></div></div>`;
...
function updateAddressDisplay() {
    ...
    if(!displayEl) return;
    let html = '';
    const buildInfo = (label, val, color) => {
        if (!val || !MEM_LOCATIONS[val]) return '';
        const loc = MEM_LOCATIONS[val];
        let h = `<div style="margin-bottom:5px; padding-bottom:5px; border-bottom:1px dashed #ccc;"><span style="font-weight:bold; color:${color};">[${label}] ${val}</span>`;
        if (loc.address) h += `<div class="address-clickable" data-address="${loc.address}" style="margin-top:2px;">📍 ${loc.address} <span style="font-size:0.8em; color:#999;">(클릭하여 복사)</span></div>`;
        if (loc.memo) h += `<div style="margin-top:2px; font-size:0.9em; color:#555;">📝 ${loc.memo}</div>`;
        h += `</div>`;
        return h;
    };
    html += buildInfo('상차', fromVal, '#007bff');
    html += buildInfo('하차', toVal, '#dc3545');
    displayEl.innerHTML = html;
    if (html !== '') displayEl.style.display = 'block'; else displayEl.style.display = 'none';
}
...
function getFormDataWithoutTime() {
    ...
    if(typeVal === '지출') {
        ...
        if(memoInput) memoVal = memoInput.value.trim();
    }
    return {
        type: typeVal,
...
function resetForm() {
    ...
    const dtField = document.getElementById('datetime-info-fieldset'); if(dtField) dtField.style.display = 'none';
    const typeLegend = document.getElementById('legend-type'); if(typeLegend) { const f = typeLegend.closest('fieldset'); if(f) f.style.display = 'none'; }
    ['transport-details', 'cost-info-fieldset'].forEach(id => { const el = document.getElementById(id); if(el) { el.classList.add('hidden'); el.style.display = 'none'; } });
    ['toggle-basic-info-btn', 'toggle-location-section-btn', 'toggle-cost-section-btn'].forEach(id => { const btn = document.getElementById(id); if(btn) btn.innerHTML = '펴기 ▼'; });
    toggleUI();
}
...
function editRecord(id) {
    ...
    if(document.getElementById('from-center')) document.getElementById('from-center').value = r.from || ''; 
    if(document.getElementById('to-center')) document.getElementById('to-center').value = r.to || '';
    if(document.getElementById('manual-distance')) document.getElementById('manual-distance').value = r.distance || ''; 
    if(document.getElementById('income')) document.getElementById('income').value = r.income ? (r.income/10000) : ''; 
    if(document.getElementById('cost')) document.getElementById('cost').value = r.cost ? (r.cost/10000) : '';
    if(document.getElementById('expense-item')) document.getElementById('expense-item').value = r.expenseItem || '';
    ...
    const dtField = document.getElementById('datetime-info-fieldset'); if(dtField) { dtField.style.display = 'block'; dtField.classList.remove('hidden'); }
    const dtBody = document.getElementById('body-datetime'); if(dtBody) dtBody.style.display = 'block'; 
    const typeLegend = document.getElementById('legend-type'); if(typeLegend) { const f = typeLegend.closest('fieldset'); if(f) { f.style.display = 'block'; f.classList.remove('hidden'); } }
    const typeBody = document.getElementById('body-type'); if(typeBody) typeBody.style.display = 'block'; 
    ...
    const fromIn = document.getElementById('from-center'); const toIn = document.getElementById('to-center');
    if(fromIn) fromIn.dispatchEvent(new Event('input')); if(toIn) toIn.dispatchEvent(new Event('input'));
    window.scrollTo({ top: 0, behavior: 'smooth' });
}

function renderFrequentLocationButtons() {
    const fromContainer = document.getElementById('top-from-centers'); const toContainer = document.getElementById('top-to-centers');
    if (!fromContainer || !toContainer) return;
    const fromCounts = {}, toCounts = {};
    MEM_RECORDS.forEach(r => { if (['화물운송','대기'].includes(r.type)) { if (r.from) fromCounts[r.from] = (fromCounts[r.from] || 0) + 1; if (r.to) toCounts[r.to] = (toCounts[r.to] || 0) + 1; } });
    const buildButtons = (data, container, targetInputId) => {
        container.innerHTML = '';
        const sorted = Object.entries(data).sort((a,b)=>b[1]-a[1]).slice(0,5);
        if (sorted.length === 0) container.style.display = 'none'; else container.style.display = 'grid'; 
        sorted.forEach(([name]) => {
            const btn = document.createElement('button'); btn.type = 'button'; btn.className = 'quick-loc-btn'; btn.textContent = name;
            btn.onclick = () => { const input = document.getElementById(targetInputId); if(input) { input.value = name; input.dispatchEvent(new Event('input')); } };
...
// ==========================================
// PART 3-1: SMS & OCR FUNCTIONS (Parsing Only)
// ==========================================

function parseSmsText() {
    const inputEl = document.getElementById('sms-input'); const input = inputEl ? inputEl.value : "";
    if (!input.trim()) { showToast("분석할 문자를 입력해주세요."); return; }
    const resultsDiv = document.getElementById('sms-parse-results'); if(!resultsDiv) return;
    resultsDiv.innerHTML = ""; resultsDiv.classList.remove('hidden'); resultsDiv.style.display = 'block';
    let allLines = input.split('\n'); if (allLines.length > 2) allLines = allLines.slice(2); 
    const validLines = allLines.filter(line => { const l = line.trim(); return l.length > 5 && !l.includes("Web발신"); });
    const sortedCenters = [...MEM_CENTERS].sort((a, b) => b.length - a.length);

    validLines.forEach((line, lineIdx) => {
        let cleaned = line.trim().replace(/\d{1,2}월\s*\d{1,2}일/g, " ").replace(/배차표|운송장/g, " ").replace(/\d+층\s*->\s*\d+층/g, " ");
        let matches = []; let searchQueue = cleaned.toUpperCase();
        sortedCenters.forEach(center => {
            const centerUpper = center.toUpperCase(); let pos = searchQueue.indexOf(centerUpper);
            if (pos !== -1) { matches.push({ name: center, index: pos }); searchQueue = searchQueue.substring(0, pos) + " ".repeat(center.length) + searchQueue.substring(pos + center.length); }
        });
        matches.sort((a, b) => a.index - b.index);
        let finalFrom = matches[0] ? matches[0].name : ""; let finalTo = matches[1] ? matches[1].name : "";
        if (!finalFrom || !finalTo) { const words = cleaned.split(/\s+/).filter(w => w.trim().length >= 2); if (!finalFrom) finalFrom = words[0] || ""; if (!finalTo) finalTo = words[1] || ""; }
...
function handleSmsNameInput(input, addrId, memoId) {
    const val = input.value.trim(); const loc = MEM_LOCATIONS[val];
    const addrInput = document.getElementById(addrId); const memoInput = document.getElementById(memoId);
    if (loc && addrInput && memoInput) { addrInput.value = loc.address || ''; memoInput.value = loc.memo || ''; addrInput.style.backgroundColor = '#f9f9f9'; } 
    else if (addrInput) { addrInput.value = ''; if(memoInput) memoInput.value = ''; addrInput.style.backgroundColor = '#fff0f0'; }
}

function registerParsedTripWithInfo(btn, lineIdx) {
    const fromName = document.getElementById(`from-${lineIdx}-name`).value.trim(); const toName = document.getElementById(`to-${lineIdx}-name`).value.trim();
    const fromAddr = document.getElementById(`from-${lineIdx}-addr`)?.value.trim(); const fromMemo = document.getElementById(`from-${lineIdx}-memo`)?.value.trim();
    const toAddr = document.getElementById(`to-${lineIdx}-addr`)?.value.trim(); const toMemo = document.getElementById(`to-${lineIdx}-memo`)?.value.trim();
    if (!fromName || !toName) { alert("상/하차지 이름을 입력해주세요."); return; }
    if (!fromAddr || !toAddr) { alert("주소 필수"); return; }
    ...
    const card = btn.closest('.sms-item-card'); if (card) card.remove(); 
    showToast(`${fromName} → ${toName} 등록됨`); if (window.updateAllDisplays) window.updateAllDisplays();
    const smsSection = document.getElementById('sms-parser-section'); if(smsSection) { smsSection.classList.remove('hidden'); smsSection.style.display = 'block'; }
}

window.onManualSmsRead = function(body) {
    const smsInput = document.getElementById('sms-input');
    if (smsInput) { smsInput.value = body; smsInput.style.height = '1px'; smsInput.style.height = (smsInput.scrollHeight) + 'px'; parseSmsText(); }
};

async function performOCR(file) {
    const statusDiv = document.getElementById('ocr-status'); const resultContainer = document.getElementById('ocr-result-container');
    if (!statusDiv || !resultContainer) return;
    statusDiv.textContent = "영수증 분석 중... (5~10초 소요)"; resultContainer.classList.add('hidden');
    try {
        const { data: { text } } = await Tesseract.recognize(file, 'kor', { logger: m => console.log(m) });
        statusDiv.textContent = "분석 완료!";
        resultContainer.innerHTML = `<div style="display:grid; grid-template-columns: 1fr 1fr; gap:5px; margin-bottom:10px;"><input type="date" id="ocr-date" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="time" id="ocr-time" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="number" id="ocr-cost" placeholder="주유금액" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="number" id="ocr-subsidy" placeholder="보조금액" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="number" id="ocr-liters" placeholder="주유리터" step="0.01" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="number" id="ocr-price" placeholder="주유단가" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="number" id="ocr-remaining" placeholder="잔여한도" step="0.01" style="padding:8px; border:1px solid #ddd; border-radius:4px;"><input type="number" id="ocr-net-cost" placeholder="실지출(자동)" readonly style="padding:8px; border:1px solid #ddd; border-radius:4px; background:#eee;"></div><div style="display:flex; gap:5px;"><button onclick="document.getElementById('ocr-result-container').classList.add('hidden')" style="flex:1; padding:10px; background:#6c757d; color:white; border:none; border-radius:4px;">초기화</button><button id="btn-save-ocr-final" style="flex:1; padding:10px; background:#28a745; color:white; border:none; border-radius:4px; font-weight:bold;">저장하기</button></div>`;
        document.getElementById('btn-save-ocr-final').addEventListener('click', saveOcrData);
        parseReceiptText(text); resultContainer.classList.remove('hidden');
    } catch (error) { console.error(error); statusDiv.textContent = "이미지 분석 실패. 다시 시도해주세요."; }
}

function parseReceiptText(text) {
    const dateMatch = text.match(/(\d{4})[-.,/년\s]+(\d{1,2})[-.,/월\s]+(\d{1,2})/);
    if (dateMatch) { document.getElementById('ocr-date').value = `${dateMatch[1]}-${dateMatch[2].padStart(2,'0')}-${dateMatch[3].padStart(2,'0')}`; } else { document.getElementById('ocr-date').value = getTodayString(); }
    const timeMatch = text.match(/(\d{1,2})\s*:\s*(\d{2})/);
    if (timeMatch) { document.getElementById('ocr-time').value = `${timeMatch[1].padStart(2,'0')}:${timeMatch[2]}`; } else { document.getElementById('ocr-time').value = getCurrentTimeString(); }
    const lines = text.split('\n');
    for (let line of lines) {
        const lineClean = line.replace(/\s/g, ''); 
        const extractNum = (str) => { const matches = str.match(/[\d,.]+/g); if (!matches) return null; let lastVal = matches[matches.length - 1]; if(lastVal.endsWith('.')) lastVal = lastVal.slice(0, -1); return parseFloat(lastVal.replace(/,/g, '')); };
...
function saveOcrData() {
    const cost = safeInt(document.getElementById('ocr-cost').value);
    if(cost <= 0) { alert('금액을 확인해주세요.'); return; }
...
// ==========================================
// PART 3-2: STATISTICS HELPER & DAILY
// ==========================================

function calculateTotalDuration(records) {
    const sorted = [...records].sort((a, b) => (a.date + a.time).localeCompare(b.date + b.time));
    let totalMinutes = 0; if (sorted.length < 2) return '0h 0m';
    for (let i = 1; i < sorted.length; i++) {
        const curr = new Date(`${sorted[i].date}T${sorted[i].time}`); const prev = new Date(`${sorted[i-1].date}T${sorted[i-1].time}`);
...
function createSummaryHTML(title, records) {
    ...
    records.forEach(r => {
        if(r.type!=='운행종료' && r.type!=='운행취소') { totalIncome += safeInt(r.income); totalExpense += safeInt(r.cost); }
        if (r.type === '주유소') { totalFuelCost += safeInt(r.cost); totalFuelLiters += safeFloat(r.liters); }
        if (['화물운송'].includes(r.type)) { totalDistance += safeFloat(r.distance); totalTripCount++; }
        if (r.type === '공차이동') { totalEmptyDist += safeFloat(r.distance); }
    });
    const netIncome = totalIncome - totalExpense;
    const items = [ { label: '수입', value: formatToManwon(totalIncome) + ' 만원', className: 'income' }, { label: '지출', value: formatToManwon(totalExpense) + ' 만원', className: 'cost' }, { label: '정산', value: formatToManwon(netIncome) + ' 만원', className: 'net' }, { label: '운행거리', value: totalDistance.toFixed(1) + ' km', className: '' }, { label: '운행건수', value: totalTripCount + ' 건', className: '' }, { label: '주유금액', value: formatToManwon(totalFuelCost) + ' 만원', className: 'cost' }, { label: '주유리터', value: totalFuelLiters.toFixed(2) + ' L', className: '' }, { label: '공차거리', value: totalEmptyDist.toFixed(1) + ' km', className: 'note' } ];
    let html = `<strong>${title}</strong><div class="summary-toggle-grid" style="display:grid; grid-template-columns: repeat(4, 1fr); gap:5px; margin-top:5px;">`;
    items.forEach(item => { html += `<div class="summary-item" style="border:1px solid #ddd; padding:5px; text-align:center; border-radius:5px;"><span class="summary-label" style="display:block; font-size:0.8em; color:#666;">${item.label}</span><span class="summary-value ${item.className || ''}" style="font-weight:bold; font-size:0.9em;">${item.value}</span></div>`; });
...
function displayTodayRecords(date) {
    ...
    const thead = todayTable.querySelector('thead'); if(thead) thead.style.display = 'none'; 
    const tbody = document.createElement('tbody'); todayTable.appendChild(tbody);
    ...
    // 정렬
    registeredList.sort((a, b) => b.id - a.id); 
    ongoingList.sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time));
    completedList.sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time)); 
    otherList.sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time));

    const createRow = (r, isHidden = false) => {
        const tr = document.createElement('tr'); tr.dataset.id = r.id; 
        if(isHidden) tr.classList.add('completed-row', 'hidden');
        ...
        if (r.time) { 
            ...
            if (idx > -1 && idx < sortedAll.length - 1) {
                const next = sortedAll[idx + 1];
                if(next.type !== '운행취소') { endTime = (next.date !== r.date) ? `(${next.date.substring(5)}) ${next.time}` : next.time; const diff = new Date(`${next.date}T${next.time}`) - new Date(`${r.date}T${r.time}`); if (diff >= 0) duration = `${Math.floor(diff / 3600000)}h ${Math.floor((diff % 3600000) / 60000)}m`; }
            }
            if(r.type === '운행취소') endTime = '취소'; else if(!endTime) endTime = '진행중';
        } else { endTime = '-'; }
        let money = ''; const inc = safeInt(r.income); const cst = safeInt(r.cost);
        if(inc > 0) money += `<span style="font-weight:bold; color:blue;">+${formatToManwon(inc)}</span> `;
        if(cst > 0) money += `<span style="font-weight:bold; color:red;">-${formatToManwon(cst)}</span>`;
        const fromLoc = MEM_LOCATIONS[r.from] || {}; const toLoc = MEM_LOCATIONS[r.to] || {};
        const fromMemoHtml = fromLoc.memo ? `<div style="color:#555; font-size:0.9em; margin-top:2px;">${fromLoc.memo}</div>` : '';
        const toMemoHtml = toLoc.memo ? `<div style="color:#555; font-size:0.9em; margin-top:2px;">${toLoc.memo}</div>` : '';
        let distInfo = r.distance ? `${safeFloat(r.distance)}km` : '';
        if(r.type === '공차이동') { tr.style.background = '#fff3cd'; distInfo = `<span style="color:#856404; font-weight:bold;">[공차]</span> ` + distInfo; }
    ...
    // [순서 변경] 등록(Registered)을 그 다음에 표시
    ...
    otherList.forEach(r => {
        const tr = document.createElement('tr'); tr.dataset.id = r.id; tr.onclick = () => editRecord(r.id);
        const money = r.cost ? `-${formatToManwon(r.cost)}` : (r.income ? `+${formatToManwon(r.income)}` : '');
        tr.innerHTML = `<td colspan="7" style="padding:10px; border-bottom:1px solid #ddd; color:#555;">[${r.type}] ${r.expenseItem || r.supplyItem || r.brand || ''} ${r.memo ? '('+r.memo+')' : ''} <span style="float:right; font-weight:bold;">${money}</span><div style="font-size:0.85em; color:#888;">${r.time}</div></td>`;
        tbody.appendChild(tr);
    });
    if(completedList.length > 0) {
        const toggleTr = document.createElement('tr'); toggleTr.innerHTML = `<td colspan="7" style="padding:0; border:none;"><button type="button" onclick="document.querySelectorAll('.completed-row').forEach(el => el.classList.toggle('hidden')); this.textContent = this.textContent.includes('보기') ? '▲ 완료된 운행 접기' : '▼ 완료된 운행 보기 (${completedList.length})';" style="width:100%; padding:12px; background:#f8f9fa; border:none; border-bottom:1px solid #ddd; color:#495057; font-weight:bold; cursor:pointer; text-align:left; font-size:1em;">▼ 완료된 운행 보기 (${completedList.length})</button></td>`;
        tbody.appendChild(toggleTr); completedList.forEach(r => tbody.appendChild(createRow(r, true)));
    }
...
function displayDailyRecords() {
    ...
    monthRecords.forEach(r => {
        const statDate = getStatisticalDate(r.date, r.time); if(!recordsByDate[statDate]) recordsByDate[statDate] = { records: [], income: 0, expense: 0, fuel: 0, distance: 0, tripCount: 0 }; recordsByDate[statDate].records.push(r);
    });

    // 2. 고정 지출 반영 (월 체크)
    MEM_FIXED_EXPENSES.forEach(fe => {
        const targetDate = `${year}-${month}-${String(fe.day).padStart(2,'0')}`;
        const targetMonthNum = parseInt(month);
        // fe.month가 0(매월)이거나, 현재 선택된 월과 같아야 함
        if (fe.month !== 0 && fe.month !== targetMonthNum) return;

        if(new Date(targetDate).getDate() === fe.day) {
            if(!recordsByDate[targetDate]) recordsByDate[targetDate] = { records: [], income: 0, expense: 0, fuel: 0, distance: 0, tripCount: 0 };
            recordsByDate[targetDate].expense += fe.cost;
        }
    });
...
function displayWeeklyRecords() {
    ...
    const weeks = {};
    monthRecords.forEach(r => {
        const statDate = getStatisticalDate(r.date, r.time), d = new Date(statDate); const w = Math.ceil((d.getDate() + (new Date(d.getFullYear(), d.getMonth(), 1).getDay())) / 7); if(!weeks[w]) weeks[w] = []; weeks[w].push(r);
    });
    Object.keys(weeks).forEach(w => {
        const data = weeks[w]; let inc = 0, exp = 0, fuel = 0, dist = 0, count = 0;
        data.forEach(r => { if(r.type === '주유소') fuel += safeInt(r.cost); else if(r.type!=='운행종료'&&r.type!=='운행취소'){ inc+=safeInt(r.income); exp+=safeInt(r.cost); } if(r.type==='화물운송'){dist+=safeFloat(r.distance);count++;} });
...
function displayMonthlyRecords() {
    ...
    MEM_FIXED_EXPENSES.forEach(fe => { 
        if(fe.month === 0) totalFixedExpense += (fe.cost * 12); // 매월
        else totalFixedExpense += fe.cost; // 특정 월 (1회)
    });
    ...
    Object.keys(months).sort().reverse().forEach(m => {
        ...
        // [월 고정 지출 합산]
        let fixedMonthly = 0;
        const currentMonthNum = parseInt(m.substring(5)); // '2025-05' -> 5
        MEM_FIXED_EXPENSES.forEach(fe => {
            if (fe.month === 0 || fe.month === currentMonthNum) {
                fixedMonthly += fe.cost;
            }
        });
        exp += fixedMonthly;

        const monthlySalary = MEM_SALARIES[m] || 0; inc += monthlySalary;
        const salaryTag = monthlySalary > 0 ? `<br><span style="font-size:0.8em; color:blue;">(급여포함)</span>` : '';
        const fixedTag = fixedMonthly > 0 ? `<br><span style="font-size:0.8em; color:red;">(고정지출)</span>` : '';
...
function displayCurrentMonthData() {
    ...
    let fixedMonthly = 0; 
    const currentMonthNum = parseInt(currentPeriod.split('-')[1]);
    MEM_FIXED_EXPENSES.forEach(fe => {
        if (fe.month === 0 || fe.month === currentMonthNum) {
            fixedMonthly += fe.cost;
        }
...
function displayCumulativeData() {
    ...
    Object.values(MEM_SALARIES).forEach(amt => inc += amt);
...
function generatePrintView(year, month, period, isDetailed) {
    ...
    const sDay = period === 'second' ? 16 : 1; const eDay = period === 'first' ? 15 : 31;
    ...
    MEM_RECORDS.forEach(r => {
        ...
        else if (['지출', '소모품', '정비'].includes(r.type)) { expenseList.push(r); sumExpGen += cost; } 
        else if (r.type === '수입') { incomeList.push(r); sumIncomeOther += income; }
    });
    ...
    const targetMonthNum = parseInt(month);
    MEM_FIXED_EXPENSES.forEach(fe => {
        // 월 체크: 0(매월) or 현재 출력 월과 일치
        if (fe.month !== 0 && fe.month !== targetMonthNum) return;

        if (fe.day >= sDay && fe.day <= eDay) {
            const dateStr = `${year}-${month}-${String(fe.day).padStart(2,'0')}`;
            // 날짜 유효성 체크
            if (new Date(dateStr).getDate() === fe.day) {
                const prefix = (fe.month && fe.month > 0) ? `[${fe.month}월 고정] ` : `[매월 고정] `;
                expenseList.push({ date: dateStr, _statDate: dateStr, time: '00:00', type: '고정지출', expenseItem: prefix + fe.name, memo: fe.memo, cost: fe.cost });
                sumExpGen += fe.cost;
            }
        }
    });

    const sortFn = (a, b) => (a.date + a.time).localeCompare(b.date + b.time);
    transportList.sort(sortFn); fuelList.sort(sortFn); expenseList.sort(sortFn); incomeList.sort(sortFn);
    ...
    const buildTable = (title, headers, rows) => {
        ...
        if (rows.length === 0) tHtml += `<tr><td colspan="${headers.length}" class="no-record">내역 없음</td></tr>`;
        else { let lastDate = ''; rows.forEach(r => { const isNewDay = r._statDate !== lastDate; lastDate = r._statDate; tHtml += `<tr class="${isNewDay ? 'new-day' : ''}">${r.cells.map(c=>`<td class="${c.cls||''}">${c.val}</td>`).join('')}</tr>`; }); }
        tHtml += `</tbody></table>`; return tHtml;
    };
    reportContent += buildTable('1. 운송 내역', 
        ...
        transportList.map(r => ({ _statDate: r._statDate, cells: [{val: r.date.substring(5), cls:'txt-center'}, {val: r.from || '-'}, {val: r.to || '-'}, {val: (r.type==='대기'?'대기':(r.type==='공차이동'?'공차이동':'화물운송')) + (isDetailed && r.expenseItem ? ` (${r.expenseItem})` : ''), cls:'txt-center'}, {val: r.distance ? r.distance+' km' : '-', cls:'txt-center'}, {val: `<input type="number" class="money-input" value="${r.income ? r.income / 10000 : 0}" step="0.1" onchange="window.updateIncomeFromPrint(this, ${r.id})">`, cls:'txt-right'}, {val: '', cls:'txt-center'} ] }))
    );
    reportContent += buildTable('2. 주유 및 정비 내역',
        [ {text:'날짜'}, {text:'주유리터'}, {text:'주유단가'}, {text:'주유금액'}, {text:'보조금액'}, {text:'실결제금액'} ],
        fuelList.map(r => { const c=safeInt(r.cost), s=safeInt(r.subsidy); return { _statDate: r._statDate, cells: [{val: r.date.substring(5), cls:'txt-center'}, {val: r.liters?r.liters.toFixed(2)+' L':'-', cls:'txt-center'}, {val: r.unitPrice?formatMoney(r.unitPrice)+' 원':'-', cls:'txt-center'}, {val: formatMoney(c)+' 원', cls:'txt-right'}, {val: '-'+formatMoney(s)+' 원', cls:'txt-right txt-red'}, {val: formatMoney(c-s)+' 원', cls:'txt-right val-bold'}]}; })
    );
    reportContent += buildTable('3. 지출 내역',
        [ {text:'날짜', style:'width:15%'}, {text:'내용 (적요)'}, {text:'지출금액', style:'width:20%'} ],
        expenseList.map(r => {
            let content = r.expenseItem||r.supplyItem||r.type;
            if(r.memo && r.memo.trim()) content += ` <span style="color:#666; font-size:0.9em;">(${r.memo})</span>`;
            if(r.type === '고정지출') content = `<span style="color:#d63384; font-weight:bold;">${content}</span>`;
            return { _statDate: r._statDate, cells: [{val: r.date.substring(5), cls:'txt-center'}, {val: content}, {val: formatMoney(safeInt(r.cost))+' 원', cls:'txt-right'}]};
        })
    );
    reportContent += buildTable('4. 수입 내역 (기타/급여)',
        [ {text:'날짜', style:'width:15%'}, {text:'내용'}, {text:'금액', style:'width:20%'} ],
        incomeList.map(r => ({ _statDate: r._statDate, cells: [{val: r.type === '급여' ? '-' : r.date.substring(5), cls:'txt-center'}, {val: r.expenseItem||'기타 수입'}, {val: formatMoney(safeInt(r.income))+' 원', cls:'txt-right txt-blue'}]}))
    );
    ...
    const oldModal = document.getElementById('print-modal'); if(oldModal) oldModal.remove();
    let modal = document.createElement('div'); modal.id = 'print-modal'; modal.style.position = 'fixed'; modal.style.top = '0'; modal.style.left = '0'; modal.style.width = '100%'; modal.style.height = '100%'; modal.style.zIndex = '9999'; modal.style.overflow = 'auto'; modal.style.background = 'white';
    modal.innerHTML = modalHtml; document.body.appendChild(modal);
    document.getElementById('btn-print-modal-close').onclick = function() { document.getElementById('print-modal').remove(); };
    document.getElementById('btn-print-modal-export').onclick = function() {
        const fileName = `CarGoNote_${year}-${month}_${periodStr.replace(/~/g,'-')}.html`;
        if(window.CargoNoteApp && window.CargoNoteApp.saveHtmlFile) { window.CargoNoteApp.saveHtmlFile(fileName, reportContent); showToast("저장 요청을 보냈습니다."); }
        else { const blob = new Blob([reportContent], { type: "text/html;charset=utf-8" }); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href = url; a.download = fileName; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url); showToast("브라우저 다운로드 시작"); }
    };
}

function displaySubsidyRecords(showAll = false) {
    const list = document.getElementById('subsidy-records-list'); if(!list) return; list.innerHTML = '';
    const allRecords = MEM_RECORDS.filter(r => r.type === '주유소').sort((a, b) => (b.date + b.time).localeCompare(a.date + a.time));
    if (allRecords.length === 0) { list.innerHTML = '<p style="text-align:center; padding:20px; color:#999;">저장된 주유 기록이 없습니다.</p>'; return; }
    const currentYM = getTodayString().substring(0, 7); 
    let displayList = []; if (showAll) { displayList = allRecords; } else { displayList = allRecords.filter(r => r.date.startsWith(currentYM)); }
    displayList.forEach(r => {
        const div = document.createElement('div'); div.className = 'center-item'; div.style.borderBottom = "1px solid #eee"; div.style.padding = "10px 0";
        div.innerHTML = `<div style="display:flex; justify-content:space-between; align-items:center;"><span style="font-weight:bold;">${r.date} <span style="font-size:0.8em; color:#666; font-weight:normal;">(${r.brand || '기타'})</span></span><span style="font-weight:bold; font-size:1.1em;">${formatToManwon(safeInt(r.cost))} 만원</span></div><div style="display:flex; justify-content:space-between; font-size:0.9em; color:#555; margin-top:4px;"><span>주유량: ${parseFloat(r.liters).toFixed(2)} L</span><span>단가: ${r.unitPrice} 원</span></div>`;
        list.appendChild(div);
    });
    if (!showAll && allRecords.length > displayList.length) { const moreBtn = document.createElement('button'); moreBtn.textContent = "▼ 이전 내역 더보기"; moreBtn.style.width = "100%"; moreBtn.style.padding = "10px"; moreBtn.style.marginTop = "10px"; moreBtn.style.background = "#f8f9fa"; moreBtn.style.border = "1px solid #ddd"; moreBtn.style.cursor = "pointer"; moreBtn.onclick = () => displaySubsidyRecords(true); list.appendChild(moreBtn); }
}

window.toggleAllSummaryValues = (container) => { container.classList.toggle('active'); container.querySelectorAll('.summary-value').forEach(el => el.classList.toggle('hidden')); };
window.viewDateDetails = (date) => { const picker = document.getElementById('today-date-picker'); if (picker) { picker.value = date; document.querySelector('.tab-btn[data-view="today"]')?.click(); updateAllDisplays(); } };
window.updateAllDisplays = function() {
    const todayPicker = document.getElementById('today-date-picker'); if(todayPicker) displayTodayRecords(todayPicker.value);
    displayDailyRecords(); displayWeeklyRecords(); displayMonthlyRecords(); displayCurrentMonthData(); renderFrequentLocationButtons(); displaySubsidyRecords(false); 
};
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    window.updateIncomeFromPrint = function(inputEl, id) {
        const newVal = parseFloat(inputEl.value); if (isNaN(newVal)) return;
        const record = MEM_RECORDS.find(r => r.id === id);
        if (record) {
            record.income = Math.round(newVal * 10000);
            if(record.type === '화물운송' && record.from && record.to) { const key = `${record.from}-${record.to}`; MEM_FARES[key] = record.income; }
            saveData(); const p = window.lastPrintParams; if(p) { generatePrintView(p.year, p.month, p.period, p.isDetailed); setTimeout(() => showToast("저장 및 재계산 완료"), 100); }
        }
    };
...
// ==========================================
    // [수정됨] 백업 실행 함수 (용량 체크 & 날짜별 폴더 저장: note/YYYY/MM/DD.json)
    // ==========================================
    function performExport(isAuto = false) {
        // 1. 저장할 데이터 구성
        const data = { 
            records: MEM_RECORDS, 
            centers: MEM_CENTERS, 
            ...
            salaries: MEM_SALARIES, 
            fixedExpenses: MEM_FIXED_EXPENSES 
        }; 
        
        const jsonString = JSON.stringify(data, null, 2);
        
        // 2. 용량 체크 (1KB 미만이면 저장 안 함 - 빈 파일 방지)
        const blob = new Blob([jsonString], { type: "application/json" });
        if (blob.size < 1024) {
            console.log(`Backup skipped: Data size (${blob.size} bytes) is too small.`);
            if (!isAuto) showToast("데이터 용량이 너무 적어(1KB 미만) 저장하지 않습니다.");
            return false;
        }

        // 3. 날짜 기반 경로 생성 (구조: note/연도/월/일.json)
        const now = new Date();
        const year = now.getFullYear();                            
        const month = String(now.getMonth() + 1).padStart(2, '0'); 
        const day = String(now.getDate()).padStart(2, '0');        

        // 예: note/2026/01/24.json
        const fullPath = `note/${year}/${month}/${day}.json`;

        // 4. Android 앱으로 데이터 전송
        if (window.CargoNoteApp && window.CargoNoteApp.saveBackupFile) { 
            // Android 앱에 데이터와 경로를 함께 전달
            window.CargoNoteApp.saveBackupFile(jsonString, fullPath); 
            
            if(isAuto) showToast(`📅 ${year}년 ${month}월 ${day}일 자동 백업 완료`); 
            else showToast(`저장 경로: /${fullPath}`); 
            return true; 
        } 
        else { 
            // 5. PC/브라우저 환경 (파일명만 변경하여 다운로드)
            if(isAuto) {
                try { 
                    const url = URL.createObjectURL(blob); 
                    const a = document.createElement('a'); 
                    a.href = url; 
                    a.download = `CargoNote_${year}-${month}-${day}.json`; 
                ...
                if(confirm("안드로이드 파일 저장이 지원되지 않는 환경입니다.\n데이터를 클립보드에 복사하시겠습니까?")) { 
                    copyTextToClipboard(jsonString, "데이터가 복사되었습니다."); 
                } 
                return false; 
            }
        }
    ...
    function checkAndAutoBackup() { 
        ...
        // 오늘 백업한 기록이 없으면 백업 실행
        if (lastBackupDate !== today) { 
            setTimeout(() => { 
                // performExport가 true(성공)를 반환할 때만 날짜 갱신
                const success = performExport(true); 
                if (success) { 
    ...
    document.getElementById('btn-save-ocr')?.addEventListener('click', () => {
        const cost = safeInt(document.getElementById('ocr-cost').value); if(cost <= 0) { alert('금액 확인 필요'); return; }
        const container = document.getElementById('ocr-result-container'); const subsidy = container.dataset.subsidy ? parseInt(container.dataset.subsidy) : 0; const brand = container.dataset.brand || '기타';
        addRecord({ id: Date.now(), date: document.getElementById('ocr-date').value, time: document.getElementById('ocr-time').value, type: '주유소', cost: cost, liters: safeFloat(document.getElementById('ocr-liters').value), unitPrice: safeInt(document.getElementById('ocr-price').value), brand: brand, subsidy: subsidy }); showToast('저장됨'); document.getElementById('ocr-result-container').classList.add('hidden'); displaySubsidyRecords(false);
    });
    ...
    document.getElementById('btn-register-trip')?.addEventListener('click', () => {
        const d = getFormDataWithoutTime(); if((d.type==='화물운송'||d.type==='대기') && (!d.from || !d.to)) return alert('상하차지 입력 필요');
        addRecord({ id: Date.now(), date: document.getElementById('date').value, time: document.getElementById('time').value, ...d }); showToast('등록되었습니다.'); resetForm(); updateAllDisplays();
    });
    document.getElementById('btn-start-trip')?.addEventListener('click', () => {
        ...
        if ((formData.type === '화물운송' || formData.type === '대기') && (!formData.from || !formData.to)) { 
            alert('상차지와 하차지를 모두 입력해주세요.'); return; 
        }

        const editId = document.getElementById('edit-id').value;
        if (editId) {
            const index = MEM_RECORDS.findIndex(r => r.id == editId); // 숫자/문자 비교 유연하게 == 사용
            if (index > -1) {
                MEM_RECORDS[index] = { 
                    ...MEM_RECORDS[index], 
                    ...formData, 
...
document.getElementById('btn-edit-end-trip')?.addEventListener('click', () => { 
        ...
        // [수정] 함수 이름 대소문자 주의 (startGPSLogging)
        if (window.CargoNoteApp && window.CargoNoteApp.startGPSLogging) {
            window.CargoNoteApp.startGPSLogging(); 
        } else {
            showToast("운행 종료 (GPS 미연동)");
        }
    ...
    document.getElementById('prev-day-btn')?.addEventListener('click', () => moveDate(-1, updateAllDisplays));
    document.getElementById('next-day-btn')?.addEventListener('click', () => moveDate(1, updateAllDisplays));
    document.getElementById('prev-daily-btn')?.addEventListener('click', () => changeDateSelect('daily-year-select', 'daily-month-select', -1, updateAllDisplays));
    document.getElementById('next-daily-btn')?.addEventListener('click', () => changeDateSelect('daily-year-select', 'daily-month-select', 1, updateAllDisplays));
    document.getElementById('prev-weekly-btn')?.addEventListener('click', () => changeDateSelect('weekly-year-select', 'weekly-month-select', -1, updateAllDisplays));
    document.getElementById('next-weekly-btn')?.addEventListener('click', () => changeDateSelect('weekly-year-select', 'weekly-month-select', 1, updateAllDisplays));
    ...
    document.querySelectorAll('.tab-btn').forEach(btn => {
        btn.addEventListener('click', (e) => {
            if(btn.parentElement.classList.contains('view-tabs')) {
                ...
                btn.classList.add('active'); document.getElementById(btn.dataset.view + '-view').classList.add('active'); updateAllDisplays();
            }
        });
    });

    document.getElementById('btn-parse-sms')?.addEventListener('click', parseSmsText);
    const smsInput = document.getElementById('sms-input'); if(smsInput) { smsInput.addEventListener('input', function() { this.style.height = '1px'; this.style.height = (this.scrollHeight)+'px'; }); }
    document.addEventListener('click', (e) => {
        const toggleHeader = e.target.closest('.collapsible-header');
        if (toggleHeader) { toggleHeader.classList.toggle('active'); const body = toggleHeader.nextElementSibling; if(body) body.classList.toggle('hidden'); if(!body.classList.contains('hidden')) { if(toggleHeader.id === 'toggle-subsidy-management') displaySubsidyRecords(false); if(toggleHeader.id === 'toggle-center-management') displayCenterList(''); } }
        let addrTarget = null; if(e.target.classList.contains('location-clickable')) { addrTarget = e.target; } else { addrTarget = e.target.closest('.address-clickable'); }
        if (addrTarget) { let addr = addrTarget.dataset.address; const centerName = addrTarget.dataset.center || addrTarget.textContent.trim(); if(!addr && centerName && MEM_LOCATIONS[centerName]) { addr = MEM_LOCATIONS[centerName].address; } if (addr) copyTextToClipboard(addr, `[${centerName}] 주소 복사됨`); else showToast(`'${centerName}' 저장된 주소 없음`); }
    });
    ...
    const addCenterBtn = document.getElementById('btn-add-center') || document.querySelector('button[onclick*="addNewCenterFromInput"]'); if (addCenterBtn) { addCenterBtn.addEventListener('click', window.addNewCenterFromInput); } 
    else { const settingsPage = document.getElementById('settings-page'); if(settingsPage) { const btns = settingsPage.querySelectorAll('button'); for(let i=0; i<btns.length; i++) { if(btns[i].textContent.includes('추가') || btns[i].textContent.includes('등록')) { btns[i].addEventListener('click', window.addNewCenterFromInput); break; } } } }

    // [설정 닫기 버튼 이벤트]
    const backBtn = document.getElementById('back-to-main-btn');
    if (backBtn) {
        backBtn.addEventListener('click', window.closeSettings);
    }

    // [거리 수신 함수 (앱 연동용)]
    window.onDistanceReceived = function(distanceKm) {
        if (!distanceKm || distanceKm <= 0) return; 
        
        const floatDist = parseFloat(distanceKm).toFixed(1);
        
        // 사용자에게 확인
        if (confirm(`측정된 거리: ${floatDist}km\n\n[확인] = 화물운송 거리로 입력\n[취소] = 공차이동으로 자동 저장`)) { 
            // 확인: 화물운송 거리 입력창에 넣기
            const distInput = document.getElementById('manual-distance'); 
            if (distInput) { 
                distInput.value = floatDist; 
                distInput.dispatchEvent(new Event('input')); 
            } 
        } else { 
            // 취소: 공차이동으로 저장할지 다시 확인
            if(confirm("공차 이동 기록으로 저장하시겠습니까?")) { 
                addRecord({ 
                    id: Date.now(), 
                    date: getTodayString(), 
                    time: getCurrentTimeString(), 
                    type: '공차이동', 
                    distance: parseFloat(floatDist), 
                    cost: 0, 
                    income: 0, 
                    from: 'GPS 측정', 
                    to: '이동' 
...
}); // <--- 여기가 파일의 끝입니다. (DOMContentLoaded 닫기)